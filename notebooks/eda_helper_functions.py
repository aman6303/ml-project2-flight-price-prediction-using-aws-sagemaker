...
def pair_plots(data, height=3, aspect=1.5, hue=None, legend=False):
    ...
    pair_grid = sns.PairGrid(
        data=data, aspect=aspect, height=height, hue=hue, corner=True
    )
...
def num_summary(data, var):
    ...
    print(
        f"{'Missing Data':15}: {col.isna().sum():,} rows ({col.isna().mean() * 100:.2f} %)"
    )
    ...
    display(
        col.quantile([0.0, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99, 1.0])
    ...
    display(
        pd.Series({"skewness": col.skew(), "kurtosis": col.kurtosis()})
        .rename("value")
        .to_frame()
    )
...
def num_univar_plots(data, var, bins=10, figsize=(15, 7)):
    ...
    # cdf
    sns.ecdfplot(data, x=var, ax=axes[1], color="red")
    axes[1].set(title="CDF")

    # power transform
    data = data.assign(
        **{f"{var}_pwt": (PowerTransformer().fit_transform(data.loc[:, [var]]).ravel())}
    )
    sns.kdeplot(data, x=f"{var}_pwt", fill=True, color="#f2b02c", ax=axes[2])
    sns.rugplot(data, x=f"{var}_pwt", color="black", height=0.035, ax=axes[2])
    axes[2].set(title="Power Transformed")

    # box plot
    sns.boxplot(data, x=var, color="#4cd138", ax=axes[3])
    axes[3].set(title="Box Plot")

    # violin plot
    sns.violinplot(data, x=var, color="#ed68b4", ax=axes[4])
    axes[4].set(title="Violin Plot")

    # qq plot
    sm.qqplot(col.dropna(), line="45", fit=True, ax=axes[5])
...
def cramers_v(data, var1, var2):
    ct = pd.crosstab(data.loc[:, var1], data.loc[:, var2])
    r, c = ct.shape
    n = ct.sum().sum()
...
def num_bivar_plots(
    data, var_x, var_y, figsize=(12, 4.5), scatter_kwargs=dict(), hexbin_kwargs=dict()
):
    ...
    # scatter plot
    sns.scatterplot(
        data, x=var_x, y=var_y, ax=axes[0], edgecolors="black", **scatter_kwargs
    )
...
def cat_summary(data, var):
    ...
    with pd.option_context("display.max_rows", None):
        display(
            col.value_counts()
            .pipe(
                lambda ser: pd.concat([ser, col.value_counts(normalize=True)], axis=1)
            )
...
def num_cat_hyp_testing(data, num_var, cat_var, alpha=0.05):
    ...
    print(
        f"- {'Alternate Hypothesis':21}: The groups don't have similar population mean"
    )
    print(f"- {'Test Statistic':21}: {statistic}")
    print(f"- {'p-value':21}: {pvalue}")
    if pvalue < alpha:
        ...
        print(
            f"- CONCLUSION: The variables {num_var} and {cat_var} are associated to each other"
        )
    else:
        print(
            f"- Since p-value is greater than {alpha}, we Fail to Reject the Null Hypothesis at {alpha * 100}% significance level"
        ...
        print(
            f"- CONCLUSION: The variables {num_var} and {cat_var} are not associated to each other"
        )
    ...
    print(
        f"- {'Alternate Hypothesis':21}: The groups don't have similar population median"
    )
    print(f"- {'Test Statistic':21}: {statistic}")
    print(f"- {'p-value':21}: {pvalue}")
    if pvalue < alpha:
...
def get_top_k(data, var, k):
    ...
    if k >= cardinality:
        raise ValueError(
            f"Cardinality of {var} is {cardinality}. K must be less than {cardinality}."
        )
    else:
        top_categories = col.value_counts(dropna=True).index[:k]
        data = data.assign(**{var: np.where(col.isin(top_categories), col, "Other")})
        return data
...
def cat_univar_plots(
    data, var, k=None, order=None, show_wordcloud=True, figsize=(12, 8.5)
):
    display_html(2, f"Univariate Analysis of {var}")
    display_html(content="")
...
def num_cat_bivar_plots(
    ...
    if orient == "v":
        ...
        # box plot
        sns.boxplot(
            data, x=cat_var, y=num_var, color="lightgreen", order=temp.index, ax=axes[1]
        )
        ...
        # box plot
        sns.boxplot(
            data, y=cat_var, x=num_var, color="lightgreen", order=temp.index, ax=axes[1]
        )
...
def cat_bivar_plots(
    data, var1, var2, k1=None, k2=None, order1=None, order2=None, figsize=(12, 8.5)
):
    ...
    # cross-tab heatmap
    ct = pd.crosstab(index=data.loc[:, var1], columns=data.loc[:, var2]).reindex(
        index=order1, columns=order2
    )
    hm = cat_heat_map(
        ct,
        mask=False,
    ...
    norm_ct = pd.crosstab(
        index=data.loc[:, var1], columns=data.loc[:, var2], normalize="index"
    ...
    # stacked bar plot
    (norm_ct.plot.bar(ax=axes[3], title="Stacked Bar Plot", stacked=True))
...
def missing_info(data):
    ...
    return (
        pd.DataFrame(
            data={"variable": na_cols, "count": na_counts, "percentage": na_pct}
        )
...
def plot_missing_info(data, bar_label_params=dict(), figsize=(10, 4)):
    ...
    ax.set(xlabel="Variable", ylabel="Count", title="Missing Data Counts per Variable")
    rotate_xlabels(ax)
    plt.tight_layout()
    plt.show()
...
def get_iqr_outliers(data, var, band=1.5):
    q1, q3 = data.loc[:, var].quantile([0.25, 0.75]).values
    ...
    return data.query(f"{var} > @upper_limit | {var} < @lower_limit").sort_values(var)
...
def dt_univar_plots(data, var, target=None, bins="auto"):
    ...
    # histogram
    sns.histplot(data=data, x=var, bins=bins, color="#1973bd", ax=ax1)
